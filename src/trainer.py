...
class Trainer():
    def __init__(self, model, config, vocab):
        ...
        self.train_data = get_examples(config.train_file, model.embedding, vocab)
        self.batch_num = int(np.ceil(len(self.train_data) / float(config.train_batch_size)))

        self.dev_data = get_examples(config.dev_file, model.embedding, vocab)
        self.test_data = get_examples(config.test_file, model.embedding, vocab)
    ...
    def _train(self, epoch):
        ...
        for batch_data in data_iter(self.train_data, self.config.train_batch_size, self.config, shuffle=True):
            ...
            loss, preds, labels, _ = self.model(batch_inputs, use_crf=epoch > self.config.crf_start_epoch)
    ...
    def _eval(self, epoch, data_nane, test_batch_size=None):
        ...
        with torch.no_grad():
            for batch_data in data_iter(data, test_batch_size, self.config, shuffle=False):
                ...
                _, preds, labels, attens = self.model(batch_inputs, use_crf=epoch > self.config.crf_start_epoch)

                for pred, label in zip(preds, labels):
                    assert len(pred) == len(label)
                    save_dic['true_label'].append(label)
                    save_dic['pred_label'].append(pred)
                    y_pred.extend(pred)
                    y_true.extend(label)
    ...
    def batch2tensor_glove(self, batch_data):
        ...
        for doc_data in batch_data:
            doc_lens.append(len(doc_data))
            max_sent_len = max(max_sent_len, max([len(sent_data[1]) for sent_data in doc_data]))

        max_doc_len = max(doc_lens)
        total_sent_num = sum(doc_lens)

        dep_graphs = []
        dis_graphs = []

        words = torch.zeros((total_sent_num, max_sent_len), dtype=torch.int64)
        extwords = torch.zeros((total_sent_num, max_sent_len), dtype=torch.int64)
        word_masks = torch.zeros((total_sent_num, max_sent_len), dtype=torch.float32)
        labels = torch.zeros((batch_size, max_doc_len), dtype=torch.int64)

        sent_idx = 0
        for bs in range(batch_size):
            for idx in range(doc_lens[bs]):
                label, word_ids, extword_ids, dep_graph = batch_data[bs][idx]

                labels[bs, idx] = label

                for word_idx in range(len(word_ids)):
                    words[sent_idx, word_idx] = word_ids[word_idx]
                    extwords[sent_idx, word_idx] = extword_ids[word_idx]
                    word_masks[sent_idx, word_idx] = 1
        ...
        return inputs, dep_graphs, doc_lens, labels
    ...
    def batch2tensor_bert(self, batch_data):
        ...
        tokens = torch.zeros((total_sent_num, max_sent_len), dtype=torch.int64)
        token_masks = torch.zeros((total_sent_num, max_sent_len), dtype=torch.float32)
        labels = torch.zeros((batch_size, max_doc_len), dtype=torch.int64)

        sent_idx = 0
        for bs in range(batch_size):
            for idx in range(doc_lens[bs]):
                label, token_ids, lens, dep_graph = batch_data[bs][idx]

                labels[bs, idx] = label

                for token_idx in range(len(token_ids)):
                    tokens[sent_idx, token_idx] = token_ids[token_idx]
                    token_masks[sent_idx, token_idx] = 1
    ...
    def batch2word(self, batch_data):
        ...
        for doc_data in batch_data:
            ...
            batch_doc_words.append(doc_words)
        return batch_doc_words
