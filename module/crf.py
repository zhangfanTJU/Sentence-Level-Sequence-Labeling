...
class CRF(nn.Module):
    ...
    def __init__(self, num_tags):
        super().__init__()
        self.base = ConditionalRandomField(num_tags, reduction='mean')
        self.top_k = 1

    def forward(self, inputs, mask, labels, reduction=None):
        bool_mask = mask.bool()
        ...
        loss = self.base(scores, labels, bool_mask, reduction) if self.training else None
        return loss, tags
